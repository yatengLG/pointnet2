...
class PointnetMSG(nn.Module):
    ...
    def forward(self, xyz:torch.Tensor, points:torch.Tensor = None):
        l1_xyz ,l1_points = self.sa1(xyz, points)               # in:3+data_channel out:64+64
        # print('l1_xyz', l1_xyz.size(), l1_points.size())
        l2_xyz, l2_points = self.sa2(l1_xyz ,l1_points)         # in:3+128  out:128+128
        # print('l2_xyz', l2_xyz.size(), l2_points.size())
        l3_xyz, l3_points = self.sa3(l2_xyz, l2_points)         # in:3+256  out:256+256
        # print('l3_xyz', l3_xyz.size() ,l3_points.size())
        l2_points = self.fp3(l2_xyz, l3_xyz, l2_points, l3_points)  # in: 256+256+256 out:256
        # print('l2_points', l2_points.size())
        l1_points = self.fp2(l1_xyz, l2_xyz, l1_points, l2_points)  # in: 128+128+128   out:128
        # print('l1_points', l1_points.size())
        l0_points = self.fp1(xyz, l1_xyz, points, l1_points)        # in: 64+64+data_channel out:128
        # print('l0_points', l0_points.size())

        x = self.drop1(F.relu(self.bn1(self.conv1(l0_points)))) # 128 ~ 128 1x1
        x = self.conv2(x)                                       # 128 ~ num_classes 1x1
        x = x.permute(0, 2, 1)
        return x
